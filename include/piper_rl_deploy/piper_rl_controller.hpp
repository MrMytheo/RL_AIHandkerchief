...
struct RobotObservation {
    ...
    // 位置信息
    std::vector<float> tennis_position;     // 网球在机械臂底座坐标系下的位置 (x, y, z)
    std::vector<float> tennis_velocity;     // 网球在机械臂底座坐标系下的速度 (vx, vy, vz)
    std::vector<float> robot_base_position;       // 机械臂底座在世界坐标系下的位置
    
    std::vector<float> tennis_world_position; // 网球在世界坐标系下的位置
...
private:
    ...
    rclcpp::Subscription<geometry_msgs::msg::PoseStamped>::SharedPtr tennis_pose_sub_;
    ...
    std::string tennis_pose_topic_;
    
    // 机械偏置参数
    float tennis_z_offset_;
    ...
    void tennisPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg);
